...
export default function TbfUsersProfile() {
    ...
    const getUserDetails = async () => {
        try {
            ...
            setFollowers(userDetails.data.followers || 0);

            // check if current user is already following this profile
            if (userDetails.data.followersList?.includes(emailLocal)) {
                setIsFollowing(true);
            }
        } catch (err) {
    ...
    const addFollowing = async () => {
        try {
            await axios.post("http://localhost:8080/plusfollowing", {
                email: emailLocal,
            });

            await axios.post("http://localhost:8080/plusfollower", {
                email: email,
            });
    ...
    const getUserPosts = async () => {
        try {
            ...
            alert("Some problem occurred while fetching posts");
            console.log(err);
        }
    };
                ...
                <div className="top h-[32%] w-full bg-gradient-to-r ">
                    ...
                    <div className="h-full w-[65%] flex flex-col justify-center">
                        <div className="h-[80%] w-full flex flex-col justify-center">
                            ...
                            <p className="font-normal text-md ml-17">
                                <i>{bio}</i>
                            </p>
                            {currUser !== username ? (
                                <button
                                    className="h-10 text-white cursor-pointer text-lg rounded-4xl font-semibold w-32 hover:bg-violet-600 ml-17 mt-5 bg-violet-700 disabled:opacity-60"
                                ...
                                <button
                                    className="h-10 text-white cursor-pointer text-lg rounded-4xl font-semibold w-28 hover:bg-violet-600 ml-17 mt-5 bg-violet-700"
                        ...
                        <div className="h-[20%] w-full flex justify-center">
                            <pre className="flex mt-5 h-full justify-center items-center w-[50%]">
                                <p className="font-semibold text-md">followers </p>
                                {followers}
                            </pre>
                            <pre className="flex mt-5 h-ful w-[50%] justify-center items-center">
                ...
                <div className="bottom min-h-[68%] max-w-full">
                    ...
                    {profilePosts.length > 0 ? (
                        ...
                        <div className="min-h-[68%] max-w-screen text-center content-center">
                            <div className="mt-10">
                                <InboxOutlinedIcon sx={{ fontSize: 60, color: "black" }} />
                            </div>
